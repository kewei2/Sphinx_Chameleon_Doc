...
path_to_webdriver = join(script_path, "chromedriver")  # This is the path to the webdriver. Change this to where you put the driver.
...
#                 import ipdb; ipdb.set_trace()
                os.system("sphinx-build -M html {} {}".format(source_folder_name, build_folder_name))
                if record[2] == 0:  # open the corresponding html for the rst file under editing
                    driver.get("file://" + html_file)
                else:
                    with io.open(html_file, 'r', encoding="utf-8") as content_file:
